...
- (void)setTransformForCurrentOrientation:(BOOL)animated {
    ...
    // Use this instead to prevent invoking [UIApplication sharedApplication] in App extension
    UIDeviceOrientation orientation = [[UIDevice currentDevice] orientation];
    CGFloat radians = 0;
    if (UIDeviceOrientationIsLandscape(orientation)) {
        if (orientation == UIDeviceOrientationLandscapeLeft) { radians = -(CGFloat)M_PI_2; }
        else { radians = (CGFloat)M_PI_2; }
        // Window coordinates differ!
        self.bounds = CGRectMake(0, 0, self.bounds.size.height, self.bounds.size.width);
    } else {
        if (orientation == UIDeviceOrientationPortraitUpsideDown) { radians = (CGFloat)M_PI; }
        else { radians = 0; }
    }
