...
@interface FilelugFileUploadService ()
...
@implementation FilelugFileUploadService {
}
...
- (void)initialConfigureWithBackgroundCompletionHandler:(void (^ _Nullable)(void))completionHandler {
    @synchronized (self) {
        ...
        if (!self.backgroundSession) {
            NSString *backgroundUploadIdentifier = [Utility backgroundUploadIdentifierForFilelug];

            NSURLSessionConfiguration *sessionConfiguration = [self sessionConfigureWithBackgroundSessionIdentifier:backgroundUploadIdentifier cachePolicy:NSURLRequestUseProtocolCachePolicy timeInterval:CONNECTION_TIME_INTERVAL_FOR_BACKGROUND_UPLOAD];
...
- (void)cancelUploadFile:(NSString *)transferKey completionHandler:(void (^ _Nullable)(void))completionHandler {
    [self.assetUploadService cancelUploadFileWithTransferKey:transferKey urlSession:self.backgroundSession completionHandler:completionHandler];
}
