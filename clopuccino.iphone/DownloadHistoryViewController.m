...
// elements of TransferHistoryModel, for downloads
...
- (void)reloadData:(id)sender {
    [self internalReloadDataWithSelectedSegmentIndex:self.selectedSegmentIndex tryAgainIfConnectionFailed:YES];
}

- (void)internalReloadDataWithSelectedSegmentIndex:(NSInteger)selectedSegmentIndex tryAgainIfConnectionFailed:(BOOL)tryAgainIfConnectionFailed {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (sessionId == nil || sessionId.length < 1) {
            ...
            [directoryService findDownloadHistoryWithTransferHistoryType:selectedSegmentIndex session:sessionId completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                self.processing = @NO;
                ...
                if (statusCode == 200) {
                    ...
                    if (parseError) {
                        ...
                        NSLog(@"Error on parsing json data as download history model array. JSON data=%@. %@, %@", dataContent, parseError, [parseError userInfo]);
                    } else {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [self.tableView reloadData];
                        });
                    }
                } else if (tryAgainIfConnectionFailed && (statusCode == 401 || (error && ([error code] == NSURLErrorUserCancelledAuthentication || [error code] == NSURLErrorSecureConnectionFailed)))) {
                    self.processing = @YES;
                    ...
                    NSString *messagePrefix = NSLocalizedString(@"Error on finding all download histories.", @"");
                    ...
                    [self.authService processCommonRequestFailuresWithMessagePrefix:messagePrefix response:response data:data error:error tryAgainAction:tryAgainAction inViewController:self reloginSuccessHandler:^(NSURLResponse *loginResponse, NSData *loginData) {
                        [self internalReloadDataWithSelectedSegmentIndex:self.selectedSegmentIndex tryAgainIfConnectionFailed:NO];
                    }];
                }
            }];
        }
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    ...
    for (TransferHistoryModel *model in histories) {
        totalBytes += [model.fileSize unsignedLongLongValue];
    }
    ...
    if (fileCount > 1) {
        return [searchText stringByAppendingFormat:@"\n%@", [NSString stringWithFormat:NSLocalizedString(@"%ld files downloaded successfully, total size: %@", @""), fileCount, displaySize]];
    } else {
        return [searchText stringByAppendingFormat:@"\n%@", [NSString stringWithFormat:NSLocalizedString(@"%ld file downloaded successfully, total size: %@", @""), fileCount, displaySize]];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *CellIdentifier = @"DownloadHistoryCell";
    ...
    if (transferHistoryModel) {
        ...
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ %@ %@\n%@ %@", displaySize, NSLocalizedString(@"Datetime prop", @""), displayDatetime, NSLocalizedString(@"from", @""), transferHistoryModel.computerName];
    }
...
//    }];
//
//    [self.authService processCommonRequestFailuresWithMessagePrefix:messagePrefix
//                                                           response:response
//                                                               data:data
//                                                              error:error
//                                                     tryAgainAction:tryAgainAction
//                                                   inViewController:self
//                                              reloginSuccessHandler:^(NSURLResponse *loginResponse, NSData *loginData) {
//        [self reloadData:nil];
//    }];
//}
